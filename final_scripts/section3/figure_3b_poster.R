...
ht_grob1 = grid.grabExpr(draw(all_plots[[1]], padding = unit(c(0, 75, 0, 10), "mm")))
ht_grob2 = grid.grabExpr(draw(all_plots[[2]], padding = unit(c(0, 75, 0, 10), "mm")))
ht_grob3 = grid.grabExpr(draw(all_plots[[3]], padding = unit(c(0, 75, 0, 10), "mm")))


png(paste0(plotDirectory, "final_figures/figure_3b_poster.png"),
    width=15, height=32, units= "in", res = 300)

grid.newpage()

top.vp <- viewport(layout=grid.layout(4, 1,
                                      widths=unit(c(1), c("null", "null", "null")),
                                      heights=unit(c(.5,5,5,5), c("null", "null", "null"))))

title1 <- viewport(layout.pos.col = 1, layout.pos.row = 1, name = "title1")
plot1 <- viewport(layout.pos.col = 1, layout.pos.row = 2, name = "plot1")
plot2 <- viewport(layout.pos.col = 1, layout.pos.row = 3, name = "plot2")
plot3 <- viewport(layout.pos.col = 1, layout.pos.row = 4, name = "plot3")
...
seekViewport("title1")
grid.text(curr_title, gp = gpar(fontsize = 30))
