...
facet_names <- c("A549 RAC Signature","Supercluster 1 Signature","Supercluster 2 Signature")
...
p <- ggboxplot(df %>% 
               ...
               panel.labs.font = list(size=14),ncol=4)
...
mean1 <- df %>% 
  filter(Recurrence=="short" & geneset=="supercluster3_signature" & treatment_stage=="post") %>% 
  pull(score) %>% 
  mean()

sd1 <- df %>% 
  filter(Recurrence=="long" & geneset=="supercluster3_signature" & treatment_stage=="post") %>% 
  pull(score) %>% 
  sd()
...
# Use only p.format as label. Remove method name.
post_plot <- p + stat_compare_means(label = "p.format", method = "wilcox")+
  ggtitle(paste0("Post-treatment"))+
  xlab("")+
  ylab("")+
  theme(legend.position="right",
        strip.text = element_text(size=20),
