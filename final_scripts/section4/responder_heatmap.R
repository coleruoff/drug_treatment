...
for(curr_geneset in genesets){
  
  for(curr_cohort in cohorts){
    
    
    r_scores <- df %>% 
      filter(geneset == curr_geneset & cohort == curr_cohort & Response == "Responder") %>% 
      pull(score)
    
    nr_scores <- df %>% 
      filter(geneset == curr_geneset & cohort == curr_cohort & Response == "Non-responder") %>% 
      pull(score)
    
    
    
    
    wilcox_res <- wilcox.test(nr_scores,r_scores, alternative = "greater")
    
    heatmap_df[["geneset"]] <- append(heatmap_df[["geneset"]],curr_geneset)
    heatmap_df[["cohort"]] <- append(heatmap_df[["cohort"]],curr_cohort)
    
    curr_color <- ifelse(mean(nr_scores) > mean(r_scores),
                         ifelse(wilcox_res$p.value < 0.05, "NR > R*","NR > R"),
                         "R > NR")
    
    
    heatmap_df[["Legend"]] <- append(heatmap_df[["Legend"]],curr_color)
...
heatmap_df$geneset <- gsub(" Signature", "", heatmap_df$geneset)

p <- ggplot(data.frame(heatmap_df), aes(x=geneset,y=cohort,fill=Legend))+
  geom_tile(color="black")+
  scale_fill_manual(values=c("red2","red4", "royalblue"))+
  xlab("")+
  ylab("")+
  ggtitle("Responder vs Non-responder Supercluster Signature Scores")+
...
png(paste0(plotDirectory,"figures/responder_nonresponder_heatmap.png"),
