...
hazard_ratio_df <- readRDS("/data/CDSL_hannenhalli/Cole/projects/drug_treatment/data/experiment_data/experiment_3_hazard_ratio_df.rds")
...
p <- ggplot(df, aes(x=hazard_ratio,y=project))+
  geom_point(position = position_dodge(width = 1),size=3, color='lightsteelblue4')+
  geom_errorbar(aes(xmin=hazard_ratio.low, xmax=hazard_ratio.high), width=1, size=2,position = "dodge",color='lightsteelblue4') +
...
res_plot <- df  |>
  ...
  ) |>  mutate(project = fct_rev(fct_relevel(project, "Project")))
...
p_right <-
  ...
  ggplot() +
  geom_text(
    aes(x = 0, y = project, label = p_value),
    hjust = 0,
...
p <- p_left + p_mid + p_right + plot_layout(design = layout) + plot_annotation("Hazard Ratios for Supercluster Signature Across TCGA Cancer Types\n",theme=theme(plot.title=element_text(hjust=0.5,face='bold',size=30)))
...
count <- 0
for(i in unique(hazard_ratio_df$project)){
  
  temp <- hazard_ratio_df %>% 
    filter(project==i, p_value < 0.05 & hazard_ratio > 0) 
