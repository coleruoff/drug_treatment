...
hazard_ratio_df <- readRDS("/data/CDSL_hannenhalli/Cole/projects/drug_treatment/data/survival_analysis_hazard_ratios/pan_cancer_deconvolved_hazard_ratios_OS.rds")
...
p <- ggplot(df, aes(x=hazard_ratio,color=signature,y=project))+
  ...
  theme_classic()+
  coord_cartesian(ylim=c(1,18),xlim=c(-20,25))+
...
p <- annotate_figure(figure, top = text_grob("Hazard Ratios for Supercluster Signatures Across Deconvolved TCGA Cancer Types\n", face = "bold", size = 36))

png("/data/ruoffcj/projects/drug_treatment/figures/deconvolved_hazard_ratios.png",
