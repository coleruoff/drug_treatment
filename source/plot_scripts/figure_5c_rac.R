...
for(curr_cell_line in cell_lines){
  ...
  data <- AddMetaData(data, metadata=scores,col.name="ecoli_score")
  
  boxplot_df <- data@meta.data %>% 
    dplyr::select(ecoli_score,rac)
...
p <- annotate_figure(figure, left = text_grob("AUCell Score", rot = 90, vjust = 1, size=35, face="bold"),
                     ...
                     top=text_grob("E. coli Antimicrobial Resistance Orthologs AUCell Scores", size=40, face="bold"))
