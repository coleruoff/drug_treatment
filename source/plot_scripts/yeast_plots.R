...
p + stat_compare_means(comparisons = my_comparisons, label = "p.format", method = "wilcox", size=8)+
  ggtitle(paste0("Yeast Stress Orthologs Mean Cluster AUCell Score (",curr_cell_line,")"))+
  xlab("")+
  ylab("AUCell Score")+
...
# Functional Enrichment of Overlapping Genes of yeast orthologs and sigantures of RAC active cells
...
Heatmap(calc_jaccard_matrix(RAC_overlap_with_yeast_signatures,RAC_overlap_with_yeast_signatures))

yeast_orthologs <- readRDS("/data/CDSL_hannenhalli/Cole/projects/drug_treatment/data/genesets/yeast_upregulated_orthologs.rds")

RAC_overlap_with_yeast_signatures <- list()
for(i in 1:length(RAC_signatures)){

  temp <- c()
  for(j in 1:length(yeast_orthologs)){

    temp <- append(temp, intersect(RAC_signatures[[i]],yeast_orthologs[[j]]))
  }
  RAC_overlap_with_yeast_signatures <- append(RAC_overlap_with_yeast_signatures, list(unique(temp)))

}

names(RAC_overlap_with_yeast_signatures) <- c("A549","K562","MCF7")

all_dotplots <- genesets_characterization(RAC_overlap_with_yeast_signatures)
...
png(paste0("/data/CDSL_hannenhalli/Cole/projects/drug_treatment/final_figures/yeast_figures/yeast_hallmarks.png"), width = curr_width,height = curr_height)
hallmarks_plt <- egg::ggarrange(plots = all_dotplots$hallmarks_dotplots, ncol = 2)
main_title <- paste0("Cancer Hallmarks Enrichment of Shared Genes Between RAC Active Cells Signature and Yeast Stress Orthologs")
hallmarks_plt <- annotate_figure(hallmarks_plt, top = text_grob(main_title, color = "black", face = "bold", size = 14))
plot(hallmarks_plt)
dev.off()

png(paste0("/data/CDSL_hannenhalli/Cole/projects/drug_treatment/final_figures/yeast_figures/yeast_mps.png"), width = curr_width,height = curr_height)
mps_plt <- egg::ggarrange(plots = all_dotplots$mps_dotplots, ncol = 2)
main_title <- paste0("ITH Meta-programs Enrichment of Shared Genes Between RAC Active Cells Signature and Yeast Stress Orthologs")
mps_plt <- annotate_figure(mps_plt, top = text_grob(main_title, color = "black", face = "bold", size = 14))
plot(mps_plt)
dev.off()

png(paste0("/data/CDSL_hannenhalli/Cole/projects/drug_treatment/final_figures/yeast_figures/yeast_functional_enrichment.png"), width = curr_width,height = curr_height)
go_plt <- egg::ggarrange(plots = all_dotplots$go_dotplots, ncol = 2)
main_title <- paste0("GO Functional Enrichment of Shared Genes Between RAC Active Cells Signature and Yeast Stress Orthologs")
