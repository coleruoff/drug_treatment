...
raj_breast_control_de_genesets <- readRDS("/data/CDSL_hannenhalli/Cole/projects/drug_treatment/data/genesets/raj_control_vs_breast_clusters_de_genesets_500.rds")

raj_breast_control_de_genes <- readRDS("/data/CDSL_hannenhalli/Cole/projects/drug_treatment/data/genesets/raj_breast_control_vs_resistant_de_genes.rds")
...
raj_breast_resistant_de_genes <- readRDS("/data/CDSL_hannenhalli/Cole/projects/drug_treatment/data/genesets/raj_breast_resistant_vs_control_de_genes.rds")
...
# plot_title <- paste0("Raj Resistant DE Signatures Enrichment in Pre-Treatment ", curr_cell_line," Clusters")
plot_title <- paste0("Raj Control DE Signatures Enrichment in Pre-Treatment ", curr_cell_line," Clusters")
...
for(i in 1:length(genesets_with_ranks)){
  ...
  curr_cluster_go_terms <- c()
  for(j in 1:length(genesets2)){
    
    genes_to_use <- gsea_results[[i]]$leadingEdge[[j]]
    ...
    curr_cluster_go_terms <- append(curr_cluster_go_terms,go_enrich@result$Description)
  }
  
  all_go_terms <- append(all_go_terms, list(unique(curr_cluster_go_terms)))
}

names(all_go_terms) <- names(gsea_results)


gsea_results[[i]]$NES
genes <- gsea_results[[i]]$leadingEdge[[1]]
